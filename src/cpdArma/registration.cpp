...
#include <cpdArma/registration.hpp>
        ...
        const arma::mat& Y) const
{
    ...
    {
        throw cpdArma::dimension_mismatch("X and Y do not have the same number of columns");
    }
    arma::mat Xn(X), Yn(Y);
...
{
    return cpdArma::find_P(X, Y, sigma2, get_outliers(), P1, Pt1, PX, use_fgt(),
                       get_epsilon());
}
